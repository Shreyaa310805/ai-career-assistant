...
export default function ApplicationReportPage() {
  ...
  return <div className="space-y-6">
    ...
    {!loading && report && (report.completed_session_count === 0 ? <Alert tone="info">Complete an interview to see your performance and skill evidence.</Alert> : <>
      ...
      <Card className="p-6"><h2 className="font-semibold">Interview improvement roadmap</h2><p className="mt-2 text-sm text-slate-500">This separate plan uses only completed interview evaluation evidence. It does not change your resume and job-description roadmap.</p>{report.interview_improvement_roadmap.length ? <ol className="mt-5 space-y-4">{report.interview_improvement_roadmap.map((step, index) => <li key={`${step.focus}-${index}`} className="flex gap-3 rounded-lg border border-line p-4"><span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-brand-50 text-sm font-semibold text-brand-700">{index + 1}</span><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-medium">{step.focus}</h3><span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${step.priority === "High" ? "bg-rose-50 text-rose-700" : "bg-amber-50 text-amber-800"}`}>{step.priority}</span>{step.score !== null ? <span className="text-xs text-slate-500">{step.score.toFixed(1)} / 100</span> : null}</div><p className="mt-1.5 text-sm text-slate-600">{step.reason}</p></div></li>)}</ol> : <p className="mt-4 text-sm text-slate-500">No assessed improvement areas yet. Complete evaluated interview answers to generate this plan.</p>}</Card>
      <Card className="p-6"><h2 className="font-semibold">Recommended practice and courses</h2><p className="mt-2 text-sm text-slate-500">Resources are recommended only for skills that need improvement in this interview report.</p>{report.interview_skill_recommendations.length ? <div className="mt-5 space-y-4">{report.interview_skill_recommendations.map(recommendation => <section key={recommendation.skill} className="rounded-lg border border-line p-4"><div className="flex flex-wrap items-center gap-2"><h3 className="font-medium">{recommendation.skill}</h3><span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${recommendation.priority === "High" ? "bg-rose-50 text-rose-700" : "bg-amber-50 text-amber-800"}`}>{recommendation.priority}</span><span className="text-xs text-slate-500">{recommendation.score.toFixed(1)} / 100</span></div><p className="mt-1.5 text-sm text-slate-600">{recommendation.reason}</p><div className="mt-3 flex flex-wrap gap-2">{recommendation.resources.map(resource => <a key={resource.url} href={resource.url} target="_blank" rel="noreferrer" className="rounded-lg border border-line px-3 py-2 text-xs font-medium text-slate-700 transition-colors hover:border-brand-300 hover:text-brand-700">{resource.title}<span className="ml-1.5 text-slate-400">{resource.provider} · {resource.type}</span></a>)}</div></section>)}</div> : <p className="mt-4 text-sm text-slate-500">No skill recommendations yet. Complete evaluated answers to receive targeted practice resources.</p>}</Card>
