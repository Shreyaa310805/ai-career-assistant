...
def list_applications(db: DbSession, current_user: CurrentUser):
    return list(db.scalars(select(Application).where(Application.user_id == current_user.id).order_by(Application.updated_at.desc())).all())


@router.post("/applications", response_model=ApplicationResponse, status_code=status.HTTP_201_CREATED)
def create_application(payload: ApplicationCreate, db: DbSession, current_user: CurrentUser):
...
def get_application(application_id: UUID, db: DbSession, current_user: CurrentUser):
    return owned_application(application_id, current_user.id, db)


@router.patch("/applications/{application_id}", response_model=ApplicationResponse)
def update_application(application_id: UUID, payload: ApplicationUpdate, db: DbSession, current_user: CurrentUser):
...
def delete_application(application_id: UUID, db: DbSession, current_user: CurrentUser):
    application = owned_application(application_id, current_user.id, db)
...
def dashboard_summary(db: DbSession, current_user: CurrentUser):
    counts = dict(db.execute(select(Application.status, func.count(Application.id)).where(Application.user_id == current_user.id).group_by(Application.status)).all())
    return DashboardSummary(
        ...
        recent_applications=list(db.scalars(select(Application).where(Application.user_id == current_user.id).order_by(Application.updated_at.desc()).limit(5)).all()),
    )
...
def premium_integration(application_id: UUID, feature: str, db: DbSession, current_user: PremiumUser):
    if feature not in {"interviews", "skill-gap", "learning"}:
